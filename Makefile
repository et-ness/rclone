...
rclone:
	go build -v --ldflags "-s -X github.com/et-ness/rclone/fs.Version=$(TAG)" $(BUILDTAGS) $(BUILD_ARGS)
	mkdir -p `go env GOPATH`/bin/
	cp -av rclone`go env GOEXE` `go env GOPATH`/bin/rclone`go env GOEXE`.new
	mv -v `go env GOPATH`/bin/rclone`go env GOEXE`.new `go env GOPATH`/bin/rclone`go env GOEXE`

test_all:
	go install --ldflags "-s -X github.com/et-ness/et-ness/fs.Version=$(TAG)" $(BUILDTAGS) $(BUILD_ARGS) github.com/et-ness/rclone/fstest/test_all
...
btest:
	@echo "[$(TAG)]($(BETA_URL)) on branch [$(BRANCH)](https://github.com/et-ness/rclone/tree/$(BRANCH)) (uploaded in 15-30 mins)" | xclip -r -sel clip
